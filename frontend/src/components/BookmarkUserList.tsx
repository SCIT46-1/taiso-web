...
function BookmarkUserList() {
  ...
  return (
    <div className="grid grid-cols-2 justify-center gap-2 mt-4 px-14">
          {bookmarkUsers.map((user) => (
            <Link to={`/users/${user.bookmarkedUserId}`} key={user.bookmarkId}
              className="w-full">
              <div
                key={user.bookmarkId}
                className="card card-side bg-base-100 shadow-md hover:shadow-lg items-center transition-shadow duration-300 relative w-full"
              >
                {/* 북마크 제거 아이콘 */}
                <svg
                  data-slot="icon"
                  ...
                  aria-hidden="true"
                  className="size-7 text-blue-500 absolute top-2 right-2 z-10 cursor-pointer m-2"
                ...
                <figure className="flex items-center justify-center w-24 h-24 p-2">
                  {user.userProfileImg ? (
                    <img
